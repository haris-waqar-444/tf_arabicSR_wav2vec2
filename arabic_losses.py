...
class ArabicCTCLoss(tf.keras.losses.Loss):
    ...
    def call(self, labels, hidden_states):
        ...
        input_length = tf.ones(self.model_input_shape[0], dtype=tf.int32) * self.model_input_shape[1]
        logit_length = tf.cast(self._get_logit_length(input_length), tf.int32)
